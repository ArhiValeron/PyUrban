...
class Pegasus(Eagle, Horse): # наследуется от Horse и Eagle в том же порядке. Таким образом выбираеться звук по умолчанию.
    # Т.е. приоритет выбора совпадающих переменных родительских классов. Я использовал обьединяющий класс для упрощения инициализации.
    # А также для того чтобы избежать не логичной на мой взляд цепочки классов в данном примере.
